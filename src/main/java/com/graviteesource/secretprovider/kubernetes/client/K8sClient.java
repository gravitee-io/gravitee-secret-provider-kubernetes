...
public class K8sClient {
    ...
    if (configuration.isClusterBased()) {
      try {
        ...
        throw new SecretManagerConfigurationException(
          "could not locate kubernetes cluster",
          e
        );
      }
    } else {
      try (
        FileReader fileReader = new FileReader(
          configuration.getKubeConfigFile()
        )
      ) {
        client =
          ClientBuilder
        ...
        throw new SecretManagerConfigurationException(
          "could not read kube configuration file",
          e
        );
      }
    }
  ...
  public Flowable<K8sSecretWatchResult> watchSecret(
    String namespace,
    String secret
  ) {
              ...
              emitter -> {
                while (watch.hasNext()) {
                  ...
                  switch (response.type) {
                    case "ADDED" -> emitter.onNext(
                      new K8sSecretWatchResult(
                        SecretEvent.Type.CREATED,
                        response.object
                      )
                    );
                    case "MODIFIED" -> emitter.onNext(
                      new K8sSecretWatchResult(
                        SecretEvent.Type.UPDATED,
                        response.object
                      )
                    );
                    case "DELETED" -> emitter.onNext(
                      new K8sSecretWatchResult(
                        SecretEvent.Type.DELETED,
                        response.object
                      )
                    );
                    case "ERROR" -> emitter.tryOnError(
                      new SecretManagerException(response.status.getMessage())
                    );
                    default -> emitter.tryOnError(
                      new IllegalStateException(
                        "Unexpected value: " + response.type
                      )
                    );
                  }
                }
  ...
  private abstract static class ApiCallbackAdapter
